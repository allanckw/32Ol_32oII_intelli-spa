 ...
 void DesignExtractor::extractDesign()
{
	...
	int totalNumOfProcs = PKB::procedures.getSize();
	for (int procNum = 0; procNum < totalNumOfProcs; procNum++) {
		ASTNode* procedureNode = (*programNode).getChild(procNum); //procedure
		...
		if (procNum != currentProc)
			throw new SPAException("Strange things abound the planet"); 
	...
	//now to recursively add back all modifies and uses relationships
	//to be continued
	/*
	...
	Note: must add in reverse cycle order.
	//*/
}
