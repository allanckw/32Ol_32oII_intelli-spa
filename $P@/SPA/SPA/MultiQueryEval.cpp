...
void MultiQueryEval::evaluate(list<string>& results)
{
    ...
    {
        list<pair<RulesOfEngagement::QueryRelations,pair<string, string>>>::iterator j = onesyno.begin();
    ...
    while (onesyno.size() > 0 || twosyno.size() > 0) {
        ...
        for (auto it = twosyno.begin(); it != twosyno.end(); it++) { //loop two syno for once
        ...
        for (auto it = onesyno.begin(); it != onesyno.end();) { //loop one syno for all
            bool toadd = false;
            if (synonymTable.isInTable(it->second.first)) {
                if (pool.find(it->second.first) != pool.end()) { //exist
                    toadd = true;
                    pool.insert(it->second.first);
                }
                } else {
                if (pool.find(it->second.second) != pool.end()) { //exist
                    toadd = true;
                    pool.insert(it->second.second);
                }
            }
            ...
            if (toadd) {
                //at this pt added to pool add to new container
                newcontainer.push_back(*it);
    ...
    for (auto it = patternAssignUsesStore.begin(); it != patternAssignUsesStore.end(); it++) {
        ...
        for (auto it2 = itsecond.begin(); it2 != itsecond.end(); it2++){
			synonymTable.setPattern((aliasMap.count(it->first) > 0) ? aliasMap[it->first] : it->first, *it2);
		}
    }
    ...
    for (auto it = zerosyno.begin(); it != zerosyno.end(); it++) {
        ...
        if (firstRel == "_") {
            ...
            if (secondRel == "_") { //Case 2: rel("1",_)
                ...
                synonymTable.setSpecificAttribute(sugar, *(RulesOfEngagement::allowableConditions[firstArg].begin()), firstRel);
