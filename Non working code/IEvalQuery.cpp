...
void IEvalQuery::cartesianUntilGoMad()
{
	...
	/*variableAnswers[firstPNode->getFirstProjectionName()] = firstPNode->getFirstProjectionAnswer();
	variableAnswers[firstPNode->getSecondProjectionName()] = firstPNode->getSecondProjectionAnswer();*/
	...
	for (int i = 1; i < projects.size(); i++)
	{
		...
		{
			if (currentPNode->getFirstProjectionName().compare(bigAnswerHeaders.at(j))) // when a matching header is found in big table
			{
				...
				for (int k = 0; k < bigAnswerIndices.at(j).size(); k++) //for every index in big table
				{ 
					for (int l = 0; l < currentPNode->getFirstProjectionAnswer().size(); l++) //for every index in current project node
					{
						if (bigAnswerIndices.at(j).at(k) == currentPNode->getFirstProjectionAnswer().at(l)) //if those indices are equal
						{
				...
				case QueryTreeNode::LastSelect:
				{
					...
					{	
						for (int i = 0; i < selected.size(); i++)
						{
							...
							selectName = selected.at(i).second;
